...
def check_components(parameters, lim_xyrel=(4 / 5, 4 / 3), delta_angle=30):
    ...
    def angle_difference(parameter, img_pca):
        ...
        return np.arccos(
            np.dot(pca.components_[0], img_pca.components_[0])
            / (
                np.linalg.norm(pca.components_[0])
                * np.linalg.norm(img_pca.components_[0])
            )
        )
    ...
    for idx in range(len(parameters)):
        if not np.all([xyrel[idx] > lim_xyrel[0], xyrel[idx] < lim_xyrel[1]]):
            dismiss[idx, 0] = True
        if not np.any(
            [angle_diff[idx] < delta_angle, angle_diff[idx] > np.pi - delta_angle]
        ):
            dismiss[idx, 1] = True
    ...
    cut_parameters = np.delete(
        parameters, np.argwhere(dismiss[:, 0] * dismiss[:, 1]).T[0], axis=0
    )
    try:
